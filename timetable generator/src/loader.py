...
def load_problem_from_directory(inputs_dir: str, optional_rooms: bool = True) -> ProblemData:
    ...
    # Normalize day_name values and preserve order of first appearance (e.g., Monday..Saturday)
    day_df["day_name"] = day_df["day_name"].astype(str).str.strip()
    unique_day_names: List[str] = []
    for name in day_df["day_name"].tolist():
        if name not in unique_day_names:
            unique_day_names.append(name)
